...
int parse (char* sData, long nDataLen, SimpleXmlTagHandler xmlTagHandler);
int parseXmlFile(char *file, SimpleXmlTagHandler xmlTagHandler);
/* ---- public api */
...
/**
 * Starts an initialized (or newly created) xml parser with the
 * specified char-array and document tag handler.
 *
 * @param sData the content to parse.
 * @param nDataLen lentgth of the content.
 * @param handler the handler to use for the document tag.
 * @return 0 if there was no error, and error code
 * > 0 if there was an error.
 */
int parse (char* sData, long nDataLen, SimpleXmlTagHandler xmlTagHandler) {
	...
	else if ((nResult=simpleXmlParse(parser, xmlTagHandler)) != 0) {
...
/**
 * Starts an initialized (or newly created) xml parser with the
 * specified file and document tag handler.
 *
...
int parseXmlFile(char *file, SimpleXmlTagHandler xmlTagHandler){
	char* sData;
	long nDataLen;
	int nResult= readFileData(file, &sData, &nDataLen);
	if (nResult != 0) {
		fprintf(stderr, "couldn't read %s (%s).\n", file, 
			getReadFileDataErrorDescription(nResult));
	} else {
		nResult = parse(sData, nDataLen, xmlTagHandler);
		free(sData);
	}
	return nResult;
}